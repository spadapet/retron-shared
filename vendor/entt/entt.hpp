...
/**
 * @brief Basic delegate2 implementation.
 *
 ...
 *
 * Unmanaged delegate2 for function pointers and members. Users of this class are
 * in charge of disconnecting instances before deleting them.
 *
 * A delegate2 can be used as a general purpose invoker without memory overhead
...
public:
    /*! @brief Function type of the delegate2. */
    using function_type = Ret(Args...);

    /*! @brief Default constructor. */
    delegate2() ENTT_NOEXCEPT
        : fn{nullptr}, data{nullptr}
    {}

    /**
     * @brief Constructs a delegate2 and connects a free function or an unbound
     * member.
     * @tparam Candidate Function or member to connect to the delegate2.
     */
    template<auto Candidate>
    delegate2(connect_arg_t<Candidate>) ENTT_NOEXCEPT
        : delegate2{}
    {
        connect<Candidate>();
    }
    ...
    /**
     * @brief Constructs a delegate2 and connects a free function with payload or
     * a bound member.
     * @tparam Candidate Function or member to connect to the delegate2.
    ...
    delegate2(connect_arg_t<Candidate>, Type &&value_or_instance) ENTT_NOEXCEPT
        : delegate2{}
    {
    ...
    /**
     * @brief Connects a free function or an unbound member to a delegate2.
     * @tparam Candidate Function or member to connect to the delegate2.
     */
    template<auto Candidate>
     ...
     * delegate2.
     *
     * The delegate2 isn't responsible for the connected object or the payload.
     * Users must always guarantee that the lifetime of the instance overcomes
     * the one  of the delegate2.<br/>
     * When used to connect a free function with payload, its signature must be
     * such that the instance is the first argument before the ones used to
     * define the delegate2 itself.
     *
     * @tparam Candidate Function or member to connect to the delegate2.
     * @tparam Type Type of class or type of payload.
    ...
    /**
     * @brief Resets a delegate2.
     *
     * After a reset, a delegate2 cannot be invoked anymore.
     */
    ...
    /**
     * @brief Returns the instance or the payload linked to a delegate2, if any.
    ...
    /**
     * @brief Triggers a delegate2.
     *
     * The delegate2 invokes the underlying function and returns the result.
     *
     * @warning
     * Attempting to trigger an invalid delegate2 results in undefined
     * behavior.<br/>
     * An assertion will abort the execution at runtime in debug mode if the
     * delegate2 has not yet been set.
     *
    ...
    /**
     * @brief Checks whether a delegate2 actually stores a listener.
     * @return False if the delegate2 is empty, true otherwise.
     */
     ...
     */
    bool operator==(const delegate2<Ret(Args...)> &other) const ENTT_NOEXCEPT {
 ...
 * @tparam Ret Return type of a function type.
 * @tparam Args Types of arguments of a function type.
 * @param lhs A valid delegate2 object.
 * @param rhs A valid delegate2 object.
 * @return True if the two contents differ, false otherwise.
 */
template<typename Ret, typename... Args>
bool operator!=(const delegate2<Ret(Args...)> &lhs, const delegate2<Ret(Args...)> &rhs) ENTT_NOEXCEPT {
    return !(lhs == rhs);
}
...
template<auto Candidate>
delegate2(connect_arg_t<Candidate>) ENTT_NOEXCEPT
-> delegate2<std::remove_pointer_t<internal::function_pointer_t<decltype(Candidate)>>>;
 ...
 */
template<auto Candidate, typename Type>
delegate2(connect_arg_t<Candidate>, Type &&) ENTT_NOEXCEPT
-> delegate2<std::remove_pointer_t<internal::function_pointer_t<decltype(Candidate), Type>>>;
...
private:
    std::vector<delegate2<Ret(Args...)>> calls;
};
...
class connection {
    ...
    connection(delegate2<void(void *)> fn, void *ref)
...
private:
    delegate2<void(void *)> disconnect;
    void *signal{};
};
...
public:
    ...
    sink before(Type *value_or_instance) {
        ...
        if(value_or_instance) {
            const auto &calls = signal->calls;
            const auto it = std::find_if(calls.cbegin(), calls.cend(), [value_or_instance](const auto &delegate2) {
                return delegate2.instance() == value_or_instance;
            });
    ...
    void disconnect(Type *value_or_instance) {
        if(value_or_instance) {
            auto &calls = signal->calls;
            calls.erase(std::remove_if(calls.begin(), calls.end(), [value_or_instance](const auto &delegate2) {
                return delegate2.instance() == value_or_instance;
            }), calls.end());
        }
    }
...
void invoke(basic_registry<Entity> &reg, const Entity entt) {
    ...
    delegate2<void(basic_registry<Entity> &, const Entity)> func;
